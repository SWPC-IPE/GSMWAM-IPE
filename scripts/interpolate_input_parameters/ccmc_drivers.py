...
DELAY_INTERVAL     = L1_DELAY # minutes
TIME_CONSTANT      = 60*5
MAX_SEARCH_DIST    = 1
DEFAULT_PATH = '.'
...
EDATE = '999901010000'
F10_TIME_DELTA = timedelta(minutes=20*60) # we are moving from 00 UT -> 20 UT, the obs time
...
class InputParameters(object):
    ...
    def __init__(self, start_date, mins, path, outfile):
        ...
        self.path      = path
        self.omni_file = path + '/imf_OMNI_{}.nc'
        self.gpi_file  = path + '/gpi_1960001-2023172.nc'
        ...
        for date in self.imf_date_list:
            try:
                ...
                if year != fd.year:
                    ...
                    nc_fid = Dataset(self.omni_file.format(year))

                idx = (dt - year_start).total_seconds() // 60

                swbz[date]  = nc_fid.variables['bz'][idx]
                swby[date]  = nc_fid.variables['by'][idx]
                swden[date] = nc_fid.variables['swden'][idx]
                swvel[date] = nc_fid.variables['swvel'][idx]
            except:
                pass
    ...
    def parse_aurora_power(self):
        for k in self.imf_date_list:
            self.hpn.dict[k]  = hemi_pow_calc(self.kp.dict[k])
            self.hps.dict[k]  = hemi_pow_calc(self.kp.dict[k])
            self.hpin.dict[k] = hpi_from_gw(self.hpn.dict[k])
            self.hpis.dict[k] = hpi_from_gw(self.hps.dict[k])

    def parse_f107_kp(self):
        f107d = self.f107.dict
        ...
        days = sorted(set([datetime.strptime(dt.strftime('%Y%m%d'),'%Y%m%d') for dt in self.f10_date_list]))

        nc_fid = Dataset(self.gpi_file)
        ...
        f107d = self.linear_int_missing_vals(f107d, self.f107.backwards_search)
        ...
        # now backfill with all available data
        for k in self.f10_date_list:
            self.f107.dict[k]  = f107d[k]
            self.f107a.dict[k] = f107a[k]
            self.ap.dict[k]    = ap[k]

        self.apa.dict = self.ap.running_average(24*60)
